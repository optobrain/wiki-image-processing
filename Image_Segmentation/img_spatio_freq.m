...
function [common_freq, common_wav,common_ang, pks_freq, pks_wav, pks_ang, add_wavs, add_angs]=img_spatio_freq(image,factor_wav,factor_ang)
    ...
    num_ang=length(ang_feat);
    % check how many additional features needed 
    left_wav=factor_wav-num_wav;
    left_ang=factor_ang-num_ang;
    % if there's need for additional features use optimal wave generator and angle generator
    if left_wav~=0
        wavMax=unique(wavMax);
        wavPks=unique(wavPks);
        add_wav=optimal_wav_gen(wavMax,wavPks,left_wav);
    end
    if left_ang~=0
        angMax=unique(angMax);
        angPks=unique(angPks);
        add_ang=optimal_ang_gen(angMax,angPks, left_ang);
    end

    figure;
    title("2D Power spectral density");
    imagesc(fx, fy, psd_dB);  % psd(fy,fx) so we don't need transpose
    [~,im]=max(psd_dB); % find out index of the maximum value
    line(fxMax, fyMax, marker="o", color='w', linestyle='none');  % mark the maximum by "o"
    line(fx_pks,fy_pks,marker="o", color='w', linestyle='none'); % mark out other peaks by "+"
    axis image
    ax = gca;
    ax.YDir = "normal";
    ax.DataAspectRatio = [1 1 1];
